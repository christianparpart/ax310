...
#     anyone to inspect.
#
# There is exactly one `file(DOWNLOAD)` in this tree -- the CPM bootstrap in
# CPM.cmake -- and one git transport, so the two settings below cover everything
# that reaches the network during a configure.
#
# Nothing enforces that. The seconds are validated here, at the definition, and a
# call site that reads a *different* name gets an empty string, which CMake
# accepts in silence: `INACTIVITY_TIMEOUT ""` is no bound at all, and the token
# being present is what makes it invisible. That is not hypothetical. CPM.cmake
# read `FASTCACHED_FETCH_SILENCE_SECONDS` long after this file was renamed to
# `AX310_`, so every configure on a machine without the bootstrap cached was
# unbounded -- the wiring there, the number gone. If a second download is ever
# added, check the name it reads against the name set below.
#
...
set(AX310_FETCH_SILENCE_SECONDS 120 CACHE STRING
    "Seconds a dependency transfer may deliver essentially nothing before it is abandoned")
set(AX310_FETCH_MIN_BYTES_PER_SECOND 1 CACHE STRING
...
# `-DAX310_FETCH_SILENCE_SECONDS=0` restores #526 exactly while every
...
foreach(fetchBoundSetting
        AX310_FETCH_SILENCE_SECONDS
        AX310_FETCH_MIN_BYTES_PER_SECOND)
...
set(ENV{GIT_HTTP_LOW_SPEED_LIMIT} "${AX310_FETCH_MIN_BYTES_PER_SECOND}")
set(ENV{GIT_HTTP_LOW_SPEED_TIME} "${AX310_FETCH_SILENCE_SECONDS}")
